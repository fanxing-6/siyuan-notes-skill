...
function parseJsonArrayEnv(value) {
    const raw = stripOptionalWrappingQuotes(value).trim();
    if (!raw) {
        return [];
    }
    ...
    try {
        ...
        throw new Error(`环境变量不是合法 JSON 数组: ${error.message}`);
    }

    if (!Array.isArray(parsed)) {
        throw new Error('环境变量必须是 JSON 数组');
    }

    return parsed;
}

function parseNotebookNameList(value) {
    const raw = stripOptionalWrappingQuotes(value).trim();
    if (!raw) {
        return [];
    }
...
function readWorkNotebookList(env) {
    return parseNotebookNameList(env.SIYUAN_WORKNOTEBOOKS || '');
}

function readConfirmedWorkNotebookList(env) {
    return parseNotebookNameList(env.SIYUAN_CONFIRMED_WORKNOTEBOOKS || '');
}
...
function buildRuntimeConfig(skillDir, env = process.env, argv = process.argv) {
    ...
    return {
        ...
        SIYUAN_WORKNOTEBOOKS: readWorkNotebookList(env),
        SIYUAN_CONFIRMED_WORKNOTEBOOKS: readConfirmedWorkNotebookList(env),
