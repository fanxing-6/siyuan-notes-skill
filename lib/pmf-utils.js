...
function trimTrailingQuoteContinuationLines(markdown) {
    const lines = String(markdown || '').split('\n');
    if (lines.length === 0) {
        return '';
    }
    ...
    while (end > 0 && /^>\s*$/.test(String(lines[end - 1] || '').trim())) {
        end -= 1;
    }
...
function inferCalloutSubType(line) {
    const match = String(line || '').trim().match(/^>\s*\[!([A-Z][A-Z0-9_-]*)\](?:\s+.*)?$/i);
    if (!match) {
        return '';
    }
...
function inferHtmlDerivedBlock(markdown) {
...
function inferBlockType(markdown) {
    ...
    const calloutSubType = inferCalloutSubType(firstLine);
    if (calloutSubType) {
        return { type: 'callout', subType: calloutSubType };
    }

    const htmlDerived = inferHtmlDerivedBlock(firstLine);
    if (htmlDerived) {
        return htmlDerived;
    }
...
function parseBlocksFromKramdown(kramdown, parentIdMap = {}, blockMetaMap = {}) {
    ...
    for (const line of lines) {
        ...
        const cleanedMarkdown = trimTrailingQuoteContinuationLines(stripKramdownIAL(markdown));
        const id = match[1];
        ...
        blocks.push({
            ...
            subType: meta.subType || inferred.subType,
            parentId: meta.parentId || parentIdMap[id] || ''
        });
        buffer = [];
    }
